...
for (const studentNumber of Array.from({length: STUDENT_COUNT}, (_, i) => i + 1)) {
  test(`Register & rate – student ${studentNumber} of ${STUDENT_COUNT}`, async ({ page }, testInfo) => {
    ...
    await page.getByRole('button', { name: "Omkar kadam anil'" }).click();
    ...
    await page1.getByLabel("View Omkar kadam anil'").getByRole('button', { name: 'Rate' }).click();
    ...
    try {
    await page1.locator('div:nth-child(6) > .flex.flex-col.sm\\:flex-row > .flex.flex-col > .flex > button:nth-child(5)')
             .click({ timeout: 3000 });
    } catch (e) {
    console.log(`[Worker ${workerIndex}] ⏭️ Specific star button not found, skipping redundant click.`);
    }
