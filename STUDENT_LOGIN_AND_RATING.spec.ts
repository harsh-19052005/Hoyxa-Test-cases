...
async function submitTeacherRating(
  ...
  await page.getByLabel(`View ${teacherName}'`).getByRole('button', { name: 'Rate' }).click();
  ...
  await page.locator(`div:nth-child(6) > .flex.flex-col.sm\\:flex-row > .flex.flex-col > .flex > button:nth-child(${ratingStar})`).click();
...
for (let idx = 0; idx < allStudents.length; idx++) {
  ...
  test(`Login & rate – student ${idx + 1}: ${cred.email}`, async ({ page, browser }) => {
    ...
    await page1.getByRole('button', { name: 'Book Session →' }).first().click();
    await page1.getByRole('button', { name: 'Career Counselling' }).click();
    await page1.getByRole('button', { name: 'Quick Select' }).click();
    await page1.getByRole('button', { name: 'Book Session', exact: true }).click();
    await page1.getByRole('button', { name: 'Pay ₹8,750.00 securely' }).click();
  });
}